...
export default function SummaryPage({ params }: SummaryPageProps) {
  ...
  const { project, dashboard, updateProject, isLoading, isError } =
    useProjectDetails(projectId);

  const { addLink, editLink, deleteLink, isPending } = useQuickLinks(projectId);

  // console.log(stats)
  const total = dashboard?.stats?.total_tasks || 0;
  const completion = dashboard?.stats?.completion_rate || 0;
  const todo = dashboard?.stats?.task_distribution?.TODO || 0;
  const inProgress = dashboard?.stats?.task_distribution?.IN_PROGRESS || 0;
  const review = dashboard?.stats?.task_distribution?.IN_REVIEW || 0;
  const done = dashboard?.stats?.task_distribution?.DONE || 0;
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in">
      ...
      <div className="lg:col-span-7 space-y-10 p-2">
        <EditableSection
          ...
          onSave={(v) => updateProject({ background: v })}
        />
        <EditableSection
          title="Objective"
          body={project?.objective || ""}
          // large -> jd gede / putih warnanya
          onSave={(v) => updateProject({ objective: v })}
        />
        <EditableSection
          title="Method"
          body={project?.method || ""}
          onSave={(v) => updateProject({ method: v })}
        />
        <EditableSection
          title="Expected Result"
          body={project?.expected_result || ""}
          onSave={(v) => updateProject({ expected_result: v })}
        />
      </div>
